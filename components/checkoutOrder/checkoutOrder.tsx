...
const CheckoutOrder = (props: Props) => {
  ...
  const incQuantityHandler = (quantity: number) => {
    ...
    if (props?.itemData?.merchandise?.quantityAvailable === productCount) {
      ...
      updateCartLineItem(createdCartID, props?.itemData?.id, quantity + 1).then((res) => {
  ...
  return (
    <>
    {
      ...
      >
        ...
        <LoadingButton loading={loadingButtonState}
